...
def create_folders(folder_path, folder_names):
    for folder in folder_names:
        path_absolute = Path(folder_path, folder)
        ...
        if not name.startswith('.'):
            for i in range(len(extensions_list)):
                if extension in extensions_list[i][1]:
                    path.rename(Path(folder_path, extensions_list[i][0], normalize(name)))
                    break
        if not name.startswith('.') and path.exists():
            path.rename(Path(folder_path, extensions_list[-1][0], normalize(name)))
...
def handle_archive(folder_path):
    archives = Path(folder_path, 'archives')
...
def make_files_list(folder_path):
    FOLDERS_DICT = {}
    for path in list(Path(folder_path).iterdir()):
...
def main():
    try:
        ...
        if Path(sort_path, 'archives').exists():
            handle_archive(sort_path)

        make_files_list(sort_path)
